...
good} relative to the design-matched block scheme (Fig.~\ref{fig:sdm}). Two further channels sharpen the
...
presence/absence; \citealp{valavi2019blockcv}). (a)~Per-point leakage map: most test
points are easier under random CV than at deployment. (b)~The CV nearest-neighbour
distances (red) sit far to the left of the deployment distances (blue): test points
are much closer to training data than the wall-to-wall map will be ($\phi$ = variogram
range). (c)~Signed SLI by candidate scheme---random CV is optimistic, buffered CV is
over-corrected into pessimism, block CV best matches deployment. (d)~Consequence: the
Brier score reported under random CV ($0.131$) is $42\%$ better than the honest,
design-matched value ($0.224$).}
\label{fig:sdm}
\end{figure}
