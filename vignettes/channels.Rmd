...
Spatial autocorrelation is only one way a train/test split can leak. `spLeakage`
provides a diagnostic for each channel, and a paired fix where one exists.
...
Repeated measurements at the same site, split across folds, leak exactly.
...
Test points close to training in *covariate* space are also easy. Supply the
deployment covariates to get a signed index.
...
If covariates are built from a focal window of radius `r`, test windows that overlap
training windows leak. The fix is a buffer of `2r`.
...
A large-scale trend that a trend-blind model cannot extrapolate is a leakage source
the autocorrelation index does not see.
...
All channels are summarised together by `audit_workflow()`.
